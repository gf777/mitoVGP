...
if ! [[ -e "${W_URL}/trimmed/${FNAME}.delta" ]]; then
	...
	nucmer --maxmatch --nosimplify ${W_URL}/freebayes_round1/${FNAME}.fasta ${W_URL}/freebayes_round1/${FNAME}.fasta -f -p "${W_URL}/trimmed/${FNAME}" -b 500
...
if ! [[ -z ${NUCMER_OUT} ]]; then
	...
	CONTIG_NEW=${W_URL}/freebayes_round1/${FNAME}.fasta

	if (( ${BEGIN2} > ${END1} )); then

		echo ">${FNAME}" > "${W_URL}/trimmed/${FNAME}_new.fasta" & grep -v ">" ${W_URL}/freebayes_round1/${FNAME} | tr -d '\n' | cut -c${BEGIN1}-${BEGIN2} >> "${W_URL}/trimmed/${FNAME}_new.fasta"

		nucmer --maxmatch --nosimplify "${W_URL}/trimmed/${FNAME}_new.fasta" "${W_URL}/trimmed/${FNAME}_new.fasta" -f -p "${W_URL}/trimmed/${FNAME}" -b 500
	...
	if (( ${BEGIN2} > ${END1} )); then
		...
		nucmer --maxmatch --nosimplify "${W_URL}/trimmed/${FNAME}_new2.fasta" "${W_URL}/trimmed/${FNAME}_new2.fasta" -f -p "${W_URL}/trimmed/${FNAME}_2" -b 500
	...
	show-coords "${W_URL}/trimmed/${FNAME}.delta" -lrcTHo
	printf "\n\nUsing: ${W_URL}/freebayes_round1/${FNAME}\n\n"
