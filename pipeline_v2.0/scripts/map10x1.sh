...
dw_date=`date "+%Y%m%d-%H%M%S"`; aws s3 --no-sign-request ls s3://genomeark/species/${SPECIES}/${ID}/genomic_data/10x/ > ${W_URL}/bowtie2_round1/log/file_list_$dw_date.txt

fi

if ! [[ -e "${W_URL}/bowtie2_round1/${ID}.1.bt2" ]]; then

bowtie2-build ${W_URL}/arrow/arrow_round2/${ID}.contig${CONTIG}_arrow2.fasta ${W_URL}/bowtie2_round1/${ID}
...
freebayes -f ${W_URL}/arrow/arrow_round2/${ID}.contig${CONTIG}_arrow2.fasta -b ${W_URL}/bowtie2_round1/aligned_${ID}_all_sorted.bam --ploidy 1 -v ${W_URL}/freebayes_round1/aligned_${ID}_all_sorted.vcf
...
if ! [[ -e "${W_URL}/freebayes_round1/${ID}.contig${CONTIG}_10x1.fasta" ]]; then

tabix -p vcf ${W_URL}/freebayes_round1/aligned_${ID}_all_sorted.vcf.gz

bcftools consensus ${W_URL}/freebayes_round1/aligned_${ID}_all_sorted.vcf.gz -f ${W_URL}/arrow/arrow_round2/${ID}.contig${CONTIG}_arrow2.fasta -o ${W_URL}/freebayes_round1/${ID}.contig${CONTIG}_arrow2_10x1.fasta
